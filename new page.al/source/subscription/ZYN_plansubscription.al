table 50198 ZYN_PlanSubscriptionTable
{
    ...
    fields
    {
        ...
        {
            DataClassification = ToBeClassified;
            TableRelation = ZYN_PlanTable."Plan ID";
            trigger OnValidate()
            begin
                CreateInvoiceAutomatically();
            end;
        ...
        field(5; Duration; Integer)
        {
            ...
            var
                nextrenewal: Integer;
            begin
                nextrenewal := Duration + 1;
                if Rec.Duration > 0 then
                    Rec."End Date" := CalcDate('<+' + Format(Rec.Duration) + 'M>', Rec."Start Date");
                Rec."Remainder Date" := Rec."End Date" - 15;

                Rec."Next renewal Date" := CalcDate('<+' + Format(nextrenewal) + 'M>', Rec."Start Date");
                // else
        ...
        field(15; "Remainder send"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(16; "Next renewal Date"; Date)
        {
            DataClassification = ToBeClassified;
        }
    }
